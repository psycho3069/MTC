        ...
        <div class="col-md-8">
            <div class="card text-left">
                <div class="card-header">
                    <b>Balance Sheet</b>
                    <form action="{{ route('report.balance') }}" method="GET" class="pull-right">
                        {{ csrf_field() }}
                        Select a date
                                                                                ...
                                                                                @foreach( $child_iii->transaction as $transaction )
                                                                                    @if( $all_bl->where('thead_id', $transaction->id)->sum('credit')>0 || $all_bl->where('thead_id', $transaction->id)->sum('debit')>0)
                                                                                        ...
                                                                                        <span>
                                                                                            ...
                                                                                            <b class="float-right">
                                                                                                @php( $balance = $all_bl->where('thead_id', $transaction->id)->sum('credit') - $all_bl->where('thead_id', $transaction->id)->sum('debit'))
                                                                                                {{ $transaction->ac_head_id == 1 ||  $transaction->ac_head_id == 4 ? -$balance : $balance }}
                                                                                            </b>
                                                                                        </span>
                                                                                        </li>
                                                                                    @endif
